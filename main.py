...
def main():
    ...
    btn_coords = [(160, 650), (480, 650), (790, 650), (1115, 650)]
    ...
    question.place(x=1280 / 2, y=200, anchor=tkinter.CENTER)
    ...
    btn1 = ttk.Button(
        root,
        text=answer,
        command=lambda: [destroy_button(btn1, btn2, btn3, btn4, question), correct()],
        style="TButton.font_regular.TButton",
    )
    btn1.place(
        x=btn_coords[0][0], y=btn_coords[0][1], anchor=tkinter.S, width=240, height=125
    )
    btn2 = ttk.Button(
        root,
    ...
    btn2.place(
        x=btn_coords[1][0], y=btn_coords[1][1], anchor=tkinter.S, width=240, height=125
    )
    btn3 = ttk.Button(
        root,
    ...
    btn3.place(
        x=btn_coords[2][0], y=btn_coords[2][1], anchor=tkinter.S, width=240, height=125
    )
    btn4 = ttk.Button(
        root,
    ...
    btn4.place(
        x=btn_coords[3][0], y=btn_coords[3][1], anchor=tkinter.S, width=240, height=125
    )
...
# ------------[Stuff]------------ #
cfg_loaded = ttk.Label(root, text=(f"Config: {cfg} Loaded"), font=font_small)

# ------------[Application Loop]------------ #
start_btn = ttk.Button(
    root, text="Start", command=main, style="TButton.font_regular.TButton"
)
start_btn.place(x=1280 / 2, y=720 / 2, anchor=tkinter.CENTER, width=200, height=50)
if cfg == "aplus":
    print(f"Loaded: {cfg}")
    cfg_loaded.place(x=65, y=15, anchor=tkinter.CENTER)
root.mainloop()
